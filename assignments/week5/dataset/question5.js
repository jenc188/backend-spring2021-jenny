...
//Street with the lowest police activitiy 
console.log("What street has the least activity?");

let lowestStreetCount;
let lowestStreetName;

for (let street in streets) {
    
    if (lowestStreetCount === undefined) {
        lowestStreetCount = streets[street];
    }

    if (streets[street] < lowestStreetCount) {

        lowestStreetCount = streets[street];
        lowestStreetName = street;
    }
}

console.log(`The street with the least police activity is ${lowestStreetName} with a total of ${lowestStreetCount} incidents.`);
